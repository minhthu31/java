...
describe("CNPM-63: Test tích hợp Dashboard", () => {
    ...
    test("1. Sử dụng đúng currentUser và bảo toàn giao diện Sprint 1", async () => {
        ...
        expect(screen.getByText("Xem trước SRS")).toBeInTheDocument();
    ...
    test("8. Team Leader mở được SRS Preview từ Dashboard", async () => {
        authService.currentUser.mockReturnValue({
            id: 3,
            username: "leader.test",
            role: "TEAM_LEADER",
        ...
        fireEvent.click(screen.getByText("Xem trước SRS"));
        expect(
            await screen.findByRole("heading", { name: "SRS Preview" }),
        ).toBeInTheDocument();
    });
});

...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        String roleName = (user.getRole() != null) ? user.getRole().getCode().name(): null;
        String token = jwtTokenProvider.generateToken(user.getUsername(), roleName);

        return new LoginResponse(token, user.getUsername(), user.getEmail(), user.getFullName(), roleName);
    }
}

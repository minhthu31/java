-- Repeatable demo seed.
-- This migration is intentionally repeatable so demo data can be enabled after
-- the database was initialized normally. Flyway recalculates the checksum when
-- the demoSeedEnabled placeholder changes.
...
-- Older CNPM-113 demo seeds used invalid RequirementStatus values.
-- Keep this cleanup limited to the CNPM-113 demo project/issues and only run when demo seed is enabled.
UPDATE requirements
SET status = 'APPROVED'
WHERE project_id IN (
  ...
  AND jira_issue_key IN ('CNPM-201', 'CNPM-202')
...
SELECT p.id, 'CNPM-201', 'Đăng nhập theo bốn vai trò',
       ...
       'Người dùng vào được màn hình phù hợp với vai trò.', 'APPROVED'
FROM projects p
...
SELECT p.id, 'CNPM-202', 'Xem báo cáo tiến độ từ Jira và GitHub',
       ...
       'Báo cáo có số liệu task, commit và Pull Request.', 'APPROVED'
FROM projects p
WHERE p.name = 'CNPM Project Management Tool'
  AND NOT EXISTS (SELECT 1 FROM requirements r

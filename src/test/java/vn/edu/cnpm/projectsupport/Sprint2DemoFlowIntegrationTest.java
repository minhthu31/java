...
class Sprint2DemoFlowIntegrationTest {
    ...
    void leaderCreatesRequirementAssignsTaskAndMemberSeesAssignedTask() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/projects/{projectId}/members", projectId)
                ...
                .andExpect(jsonPath("$.data[*].username", hasItem("member.test")));
        mockMvc.perform(get("/api/v1/projects/{projectId}/members", projectId)
                .header(HttpHeaders.AUTHORIZATION, bearer(memberToken)))
                .andExpect(status().isForbidden());

        String requirementResponse = mockMvc.perform(post("/api/v1/projects/{projectId}/requirements", projectId)
                        .header(HttpHeaders.AUTHORIZATION, bearer(leaderToken))
        ...
        String taskResponse = mockMvc.perform(post("/api/v1/projects/{projectId}/tasks", projectId)
                        .header(HttpHeaders.AUTHORIZATION, bearer(leaderToken))
        ...
        mockMvc.perform(get("/api/v1/projects/{projectId}/tasks", projectId).header(HttpHeaders.AUTHORIZATION, bearer(leaderToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.content[?(@.id == %s)]", taskId).exists());

        mockMvc.perform(get("/api/v1/projects/{projectId}/tasks", projectId).header(HttpHeaders.AUTHORIZATION, bearer(memberToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalElements").value(2))
                .andExpect(jsonPath("$.data.content[?(@.id == %s)]", taskId).exists());

        mockMvc.perform(get("/api/v1/projects/{projectId}/requirements", projectId).header(HttpHeaders.AUTHORIZATION, bearer(leaderToken)))
                .andExpect(status().isOk())
    ...
    private JsonNode login(String username, String password) throws Exception {
        String response = mockMvc.perform(post("/api/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(Map.of("usernameOrEmail", username, "password", password))))
                .andExpect(status().isOk())
